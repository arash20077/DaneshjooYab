...
function App() {
  ...
  const openAuth = (mode: 'login' | 'register') => { setAuthMode(mode); setAuthStep('email'); setEmail(''); setName(''); setCode(''); setAgreed(false); setDevCode(''); setAuthOpen(true); };
  ...
  const requestCode = async (event: FormEvent) => {
    ...
    setSendingCode(true);
    try { const data = await api<{ message: string; devCode?: string }>('/api/auth/request-code', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: cleaned, purpose: authMode }) }); setEmail(cleaned); setDevCode(data.devCode || ''); setAuthStep('code'); }
    catch (e) { setMessage((e as Error).message); }
    finally { setSendingCode(false); }
  };

  const verifyCode = async (event: FormEvent) => {
    event.preventDefault(); setVerifying(true); setMessage('');
    try { const data = await api<{ user: Account }>('/api/auth/verify-code', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email: email.trim().toLowerCase(), code, name }) }); setAccount(data.user); setAuthOpen(false); setMessage('ورود با موفقیت انجام شد.'); }
    catch (e) { setMessage((e as Error).message); }
  ...
  return <main className="min-h-screen bg-slate-50 text-slate-800">
    ...
    {authOpen && <div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/35 p-4"><form onSubmit={authStep === 'email' ? requestCode : verifyCode} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl"><div className="flex items-center justify-between"><h2 className="text-xl font-black">{authMode === 'login' ? 'ورود به حساب' : 'ثبت‌نام'}</h2><button type="button" onClick={() => setAuthOpen(false)} aria-label="بستن" className="rounded-lg p-2 hover:bg-slate-100"><X /></button></div>{authStep === 'email' ? <><p className="mt-2 text-sm text-slate-500">یک کد ۶ رقمی به ایمیل شما ارسال می‌شود.</p>{authMode === 'register' && <label className="mt-5 block text-sm font-bold">نام نمایشی<input value={name} onChange={e => setName(e.target.value)} className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3" /></label>}<label className="mt-4 block text-sm font-bold">ایمیل<input type="email" value={email} onChange={e => setEmail(e.target.value)} dir="ltr" required className="mt-2 min-h-11 w-full rounded-xl border border-slate-200 px-3" /></label>{authMode === 'register' && <div className="mt-4 rounded-xl bg-slate-50 p-4 text-xs leading-6 text-slate-600"><p>این وب‌سایت پروژه‌ای آموزشی و پژوهشی است و سیستم رسمی دانشگاه نیست مگر صریحاً اعلام شود.</p><label className="mt-3 flex items-start gap-2 font-bold text-slate-700"><input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} className="mt-1" />شرایط استفاده و قوانین سایت را مطالعه کرده‌ام و با آن موافقم.</label></div>}<button disabled={sendingCode || (authMode === 'register' && !agreed)} className="mt-5 min-h-12 w-full rounded-xl bg-indigo-600 font-bold text-white disabled:opacity-40">{sendingCode ? 'در حال ارسال...' : 'ارسال کد تأیید'}</button></> : <><p className="mt-2 text-sm text-slate-500">کد ارسال‌شده به <b dir="ltr">{email}</b> را وارد کنید.</p><input autoFocus inputMode="numeric" maxLength={6} value={code} onChange={e => setCode(e.target.value.replace(/\D/g, ''))} dir="ltr" className="mt-5 min-h-14 w-full rounded-xl border border-slate-200 px-4 text-center text-2xl font-black tracking-[0.5em]" placeholder="••••••" />{devCode && <p className="mt-3 rounded-xl bg-amber-50 p-3 text-center text-xs text-amber-800">حالت توسعه: کد {devCode}</p>}<button disabled={verifying || code.length !== 6} className="mt-5 min-h-12 w-full rounded-xl bg-indigo-600 font-bold text-white disabled:opacity-40">{verifying ? 'در حال بررسی...' : 'تأیید و ورود'}</button><button type="button" onClick={() => setAuthStep('email')} className="mt-3 w-full text-sm font-bold text-indigo-700">تغییر ایمیل</button></>}{authStep === 'email' && <button type="button" onClick={() => { setAuthMode(authMode === 'login' ? 'register' : 'login'); setAgreed(false); }} className="mt-4 w-full text-sm font-bold text-indigo-700">{authMode === 'login' ? 'حساب ندارید؟ ثبت‌نام کنید' : 'حساب دارید؟ وارد شوید'}</button>}</form></div>}
