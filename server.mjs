...
async function sendOtpEmail(email, code) {
  ...
  if (!apiKey || !from) {
    if (isProduction) throw new Error('سرویس ایمیل روی سرور تنظیم نشده است.');
    console.log(`[DEV OTP] ${email}: ${code}`);
    return;
  }
  const response = await fetch('https://api.resend.com/emails', {
    method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ from, to: [email], subject: 'کد ورود دانشجویاب', html: `<div dir="rtl" style="font-family:Arial,sans-serif"><h2>کد تأیید دانشجویاب</h2><p>کد شما:</p><p style="font-size:32px;font-weight:700;letter-spacing:8px">${code}</p><p>این کد ۱۰ دقیقه اعتبار دارد. اگر شما این درخواست را نداده‌اید، این پیام را نادیده بگیرید.</p></div>` })
  });
  if (!response.ok) throw new Error('ارسال ایمیل ناموفق بود.');
}

async function requestCode(req, res) {
  if (!OTP_SECRET) return json(res, 500, { error: 'OTP_SECRET تنظیم نشده است.' });
  const body = await readJson(req);
  const email = cleanEmail(body.email);
  const purpose = body.purpose === 'register' ? 'register' : 'login';
  if (!validEmail(email)) return json(res, 400, { error: 'ایمیل معتبر وارد کنید.' });
  const ip = req.socket.remoteAddress || 'unknown';
  if (!rateLimit(`otp-email:${email}`, 3, 15 * 60 * 1000) || !rateLimit(`otp-ip:${ip}`, 10, 15 * 60 * 1000)) return json(res, 429, { error: 'درخواست‌های زیادی ارسال شده است. کمی بعد دوباره تلاش کنید.' });
  db.prepare('UPDATE otp_codes SET consumed=1 WHERE email=? AND consumed=0').run(email);
  const code = String(crypto.randomInt(0, 1000000)).padStart(6, '0');
  db.prepare('INSERT INTO otp_codes(email,purpose,code_hash,expires_at,created_at) VALUES(?,?,?,?,?)').run(email, purpose, otpHash(email, code), Date.now() + OTP_TTL_MS, Date.now());
  try { await sendOtpEmail(email, code); } catch (e) { db.prepare('UPDATE otp_codes SET consumed=1 WHERE email=? AND consumed=0').run(email); throw e; }
  json(res, 200, { ok: true, message: 'کد تأیید ارسال شد.', ...(isProduction ? {} : { devCode: code }) });
}

async function verifyCode(req, res) {
  const body = await readJson(req);
  const email = cleanEmail(body.email); const code = String(body.code || '').trim();
  const name = normalizeName(body.name || '') || 'کاربر دانشجو';
  if (!validEmail(email) || !/^\d{6}$/.test(code)) return json(res, 400, { error: 'ایمیل یا کد تأیید معتبر نیست.' });
  const item = db.prepare('SELECT * FROM otp_codes WHERE email=? AND consumed=0 AND expires_at>? ORDER BY id DESC LIMIT 1').get(email, Date.now());
  if (!item) return json(res, 400, { error: 'کد تأیید منقضی شده یا وجود ندارد.' });
  if (item.attempts >= 5) return json(res, 429, { error: 'تعداد تلاش‌های مجاز تمام شده است. کد جدید بگیرید.' });
  const expected = otpHash(email, code);
  if (!safeEqualHex(expected, item.code_hash)) { db.prepare('UPDATE otp_codes SET attempts=attempts+1 WHERE id=?').run(item.id); return json(res, 400, { error: 'کد تأیید نادرست است.' }); }
  db.prepare('UPDATE otp_codes SET consumed=1 WHERE id=?').run(item.id);
  let user = db.prepare('SELECT * FROM users WHERE email=?').get(email);
  if (!user) {
    user = { id: randomId(), email, name, role: email === ADMIN_EMAIL ? 'admin' : 'user', created_at: new Date().toISOString() };
    db.prepare('INSERT INTO users(id,email,name,role,created_at) VALUES(?,?,?,?,?)').run(user.id, user.email, user.name, user.role, user.created_at);
  } else if (name && name !== 'کاربر دانشجو') {
    db.prepare('UPDATE users SET name=? WHERE id=?').run(name, user.id); user = { ...user, name };
  }
  const token = crypto.randomBytes(32).toString('base64url');
  db.prepare('INSERT INTO sessions(token_hash,user_id,expires_at,created_at) VALUES(?,?,?,?)').run(hash(token), user.id, Date.now() + SESSION_TTL_MS, Date.now());
...
async function route(req, res) {
  ...
  try {
    ...
    if (method === 'POST' && url.pathname === '/api/auth/request-code') return requestCode(req, res);
    if (method === 'POST' && url.pathname === '/api/auth/verify-code') return verifyCode(req, res);
...
if (isProduction && !OTP_SECRET) console.error('WARNING: OTP_SECRET is missing. Authentication requests will fail.');
